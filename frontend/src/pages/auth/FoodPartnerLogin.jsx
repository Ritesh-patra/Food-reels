...
const FoodPartnerLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post("https://food-reels.onrender.com/api/auth/food-partner/login",
         { email, password },{
    ...
    <main className="h-[90vh] p-3 py-6 flex items-center justify-center bg-background">
      <div className="w-full max-w-md rounded-xl border bg-card text-card-foreground shadow">
        <header className="flex items-center justify-between gap-4 border-b px-6 py-5">
          <div>
            <h1 className="text-pretty text-lg font-semibold">Partner sign in</h1>
            <p className="text-sm text-muted-foreground">Access your partner dashboard.</p>
          </div>
  ...
  Food partner
        ...
        <footer className="border-t px-6 py-5 flex flex-col gap-3">
          <p className="text-sm text-muted-foreground text-center">
            New partner?{" "}
            <Link to="/food-partner/register" className="underline underline-offset-4 hover:text-foreground">
              Create an account
            </Link>
          </p>
        </footer>
